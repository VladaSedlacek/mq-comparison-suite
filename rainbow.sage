...
def Keygen(q, n, m, o2):
    ...
    while not T.is_invertible():
        T = Matrix(K, [[K.random_element() for j in range(m)]
                       for i in range(m)])

    S = Matrix(K, n, n)
    while not S.is_invertible():
        S = Matrix(K, [[K.random_element() for j in range(n)]
                       for i in range(n)])
    ...
    O1 = S.inverse() * Matrix(K, [basis_Fn[i]
                                  for i in range(n - m, n)]).transpose()
    O2 = S.inverse() * Matrix(K, [basis_Fn[i]
                                  for i in range(n - o2, n)]).transpose()
